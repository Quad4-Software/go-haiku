...
haiku_load_bootstrap_lock() {
	...
	while IFS= read -r line || [ -n "$line" ]; do
		...
		val=${line#*=}
		case "$key" in
		HAIKU_BOOTSTRAP_URL|HAIKU_BOOTSTRAP_SHA256|HAIKU_BOOTSTRAP_URL_386|HAIKU_BOOTSTRAP_SHA256_386|HAIKU_BOOTSTRAP_TAG|HAIKU_BOOTSTRAP_ALLOWED_HOSTS|HAIKU_BOOTSTRAP_ALLOWED_REPOS)
			eval "cur=\${$key-}"
			if [ -z "$cur" ]; then
				export "$key=$val"
			fi
